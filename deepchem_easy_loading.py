...
## testing phase
"""
Unit tests for the DeepChem Easy Model Loading module.

This module contains comprehensive tests for the model saving, loading, and
transfer learning functionality, with special focus on Morgan fingerprint support.
"""
...
from deepchem_easy_loading import (
    save_model,
    load_pretrained,
    create_model_from_pretrained,
    get_model_metadata,
    create_morgan_featurizer,
    EasyModelHub,
    register_models_and_featurizers
)
...
class TestModelSaveLoad(unittest.TestCase):
    """Test cases for model saving and loading functionality."""

    def setUp(self):
        """Set up test models and temporary directory."""
        # Create a temporary directory for model saving
        self.temp_dir = tempfile.mkdtemp()

        # Create a simple featurizer
        self.featurizer = CircularFingerprint(size=1024, radius=2)
    ...
    def test_save_model_default(self):
        """Test saving a model with default settings."""
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_default"),
        ...
        self.assertTrue(os.path.exists(os.path.join(save_path, "model")))

        # Check that featurizer.pkl was created
        self.assertTrue(os.path.exists(os.path.join(save_path, "featurizer.pkl")))

    def test_save_model_pickle(self):
        """Test saving a model using pickle format."""
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_pickle"),
            featurizer=self.featurizer,
            save_format="pickle"
        )

        # Check that pickled_model.pkl was created
        self.assertTrue(os.path.exists(os.path.join(save_path, "pickled_model.pkl")))
    ...
    def test_save_model_with_metadata(self):
        ...
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_with_metadata"),
            metadata=custom_metadata
        )

        # Load metadata and verify custom fields
        with open(os.path.join(save_path, "metadata.json"), "r") as f:
            metadata = json.load(f)

        self.assertIn("user_metadata", metadata)
        self.assertEqual(metadata["user_metadata"]["author"], "Test Author")

    def test_load_pretrained_default(self):
        """Test loading a model that was saved with default settings."""
        # First save a model
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_to_load"),
            featurizer=self.featurizer
        )

        # Now load it
        loaded_model, loaded_featurizer = load_pretrained(save_path)

        # Verify model class
        self.assertIsInstance(loaded_model, MultitaskClassifier)

        # Verify model parameters
        self.assertEqual(loaded_model.n_tasks, 2)
        self.assertEqual(loaded_model.n_features, 1024)

        # Verify featurizer class
        self.assertIsInstance(loaded_featurizer, CircularFingerprint)

        # Verify featurizer parameters
        self.assertEqual(loaded_featurizer.size, 1024)
        self.assertEqual(loaded_featurizer.radius, 2)

    def test_load_pretrained_without_featurizer(self):
        """Test loading a model without loading the featurizer."""
        # First save a model with a featurizer
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_no_feat"),
            featurizer=self.featurizer
        )

        # Now load only the model
        loaded_model = load_pretrained(save_path, load_featurizer=False)

        # Verify model class
        self.assertIsInstance(loaded_model, MultitaskClassifier)

        # Verify no featurizer was returned
        self.assertFalse(isinstance(loaded_model, tuple))

    def test_create_model_from_pretrained(self):
        """Test creating a new model instance from pretrained weights."""
        # First save a model
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_for_transfer")
        )

        # Create a new model with different parameters
        new_model = create_model_from_pretrained(
            save_path,
            n_tasks=3,  # Changed from 2
            dropouts=0.2  # Changed from 0.1
        )

        # Verify modified parameters
        self.assertEqual(new_model.n_tasks, 3)
        self.assertEqual(new_model.dropouts, 0.2)

        # Original parameters should be preserved
        self.assertEqual(new_model.n_features, 1024)


class TestMorganFingerprintSupport(unittest.TestCase):
    """Test cases for Morgan fingerprint specific functionality."""

    def setUp(self):
        """Set up test models and temporary directory."""
        self.temp_dir = tempfile.mkdtemp()

        # Create Morgan fingerprint featurizer with various parameters
        self.morgan_featurizer = CircularFingerprint(
            size=2048,
            radius=3,
            use_chirality=True,
            use_features=True
    ...
    def test_morgan_metadata(self):
        """Test that Morgan fingerprint parameters are properly saved in metadata."""
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "morgan_model"),
            featurizer=self.morgan_featurizer
        )

        # Load metadata and verify Morgan fingerprint details
        with open(os.path.join(save_path, "metadata.json"), "r") as f:
            metadata = json.load(f)

        self.assertIn("featurizer", metadata)
        self.assertEqual(metadata["featurizer"]["type"], "morgan")
        self.assertEqual(metadata["featurizer"]["radius"], 3)
        self.assertEqual(metadata["featurizer"]["size"], 2048)
        self.assertEqual(metadata["featurizer"]["chiral"], True)
        self.assertEqual(metadata["featurizer"]["features"], True)

    def test_load_morgan_featurizer(self):
        """Test loading a model with Morgan fingerprint featurizer."""
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "model_with_morgan"),
            featurizer=self.morgan_featurizer
        )

        # Now load it
        _, loaded_featurizer = load_pretrained(save_path)

        # Verify featurizer parameters
        self.assertEqual(loaded_featurizer.radius, 3)
        self.assertEqual(loaded_featurizer.size, 2048)
        self.assertEqual(getattr(loaded_featurizer, "use_chirality", False), True)
        self.assertEqual(getattr(loaded_featurizer, "use_features", False), True)

    def test_create_morgan_featurizer(self):
        """Test the convenience function for creating Morgan fingerprints."""
        ...
        # Verify featurizer parameters
        self.assertEqual(featurizer.radius, 4)
        self.assertEqual(featurizer.size, 1024)
        self.assertEqual(getattr(featurizer, "use_chirality", False), True)
        self.assertEqual(getattr(featurizer, "use_features", False), False)


class TestModelHub(unittest.TestCase):
    """Test cases for the EasyModelHub class."""

    def setUp(self):
        """Set up test models and hub directory."""
        self.temp_dir = tempfile.mkdtemp()
        self.hub = EasyModelHub(models_dir=self.temp_dir)
        ...
        # Save it directly to the hub directory
        self.model_id = "test_model"
        save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, self.model_id)
        )
    ...
    def test_list_local_models(self):
        ...
        models = self.hub.list_local_models()

        # Should contain our test model
        self.assertIn(self.model_id, models)

    def test_load_model(self):
        """Test loading a model from the hub."""
        loaded_model = self.hub.load_model(self.model_id, load_featurizer=False)

        # Verify model class and parameters
        self.assertIsInstance(loaded_model, MultitaskClassifier)
        self.assertEqual(loaded_model.n_tasks, 1)
        self.assertEqual(loaded_model.n_features, 128)

    def test_model_not_found(self):
        """Test error handling when model is not found."""
        with self.assertRaises(ValueError):
            self.hub.load_model("nonexistent_model")
...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_get_model_metadata(self):
        """Test retrieving model metadata without loading the model."""
        # First save a model
        save_path = save_model(
            model=self.model,
            save_dir=os.path.join(self.temp_dir, "metadata_model"),
            metadata={"test": "value"}
        )

        # Get metadata
        metadata = get_model_metadata(save_path)

        # Verify key fields
        self.assertEqual(metadata["model_class"], "MultitaskClassifier")
        self.assertEqual(metadata["user_metadata"]["test"], "value")

    def test_register_models_and_featurizers(self):
        """Test model and featurizer registration."""
        # Clear registries first
        from deepchem_easy_loading import MODEL_REGISTRY, FEATURIZER_REGISTRY
        MODEL_REGISTRY.clear()
        FEATURIZER_REGISTRY.clear()

        # Register models and featurizers
        register_models_and_featurizers()

        # Check that common models and featurizers are registered
        self.assertIn("MultitaskClassifier", MODEL_REGISTRY)
        self.assertIn("CircularFingerprint", FEATURIZER_REGISTRY)